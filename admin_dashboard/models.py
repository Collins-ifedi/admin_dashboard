...
class User(Base):
    ...
    # Relationships
    # UPDATED: Changed lazy="dynamic" to lazy="selectin" for Async compatibility.
    # "dynamic" returns a Sync Query object which fails in async contexts.
    orders = relationship("Order", back_populates="user", lazy="selectin")
    topups = relationship("TopUp", back_populates="user", lazy="selectin")
...
class Product(Base):
    ...
    # Relationships
    # UPDATED: Changed default lazy load to "selectin" to prevent implicit I/O errors.
    codes = relationship("ProductCode", back_populates="product", cascade="all, delete-orphan", lazy="selectin")
    stock_alerts = relationship("ProductStockAlert", back_populates="product", cascade="all, delete-orphan", lazy="selectin")
...
class Order(Base):
    ...
    # Relationships
    # These often load with "joined" or "selectin" by default if accessed frequently,
    # but strictly defining them isn't as critical as the collection attributes above.
...
# =========================
# NEW MODELS (Broadcasts, Notifications, Alerts)
# =========================

class BroadcastMessage(Base):
    """
    Stores messages sent to multiple users.
    """
    ...
    # Flags to determine audience
    sent_to_all = Column(Boolean, default=False, nullable=False)
    
    # Stores a JSON string or comma-separated list of Telegram IDs if not sent_to_all
    sent_to_user_ids = Column(Text, nullable=True) 

    def __repr__(self):
        return f"<Broadcast id={self.id} all={self.sent_to_all}>"


class Notification(Base):
    """
    In-app notifications for users or admins.
    Used for order confirmations, low stock warnings, etc.
    """
    ...
    # If null, it might be a system-wide alert or intended for all admins
    ...
    # Index for quickly fetching unread notifications for a user
    __table_args__ = (
...
class ProductStockAlert(Base):
    """
    Configuration for when to alert admins about low stock for specific products.
    """
    ...
    # To prevent spamming alerts, track the last time we notified admins
    last_alert_sent_at = Column(DateTime(timezone=True), nullable=True)

    product = relationship("Product", back_populates="stock_alerts")

    def __repr__(self):
        return f"<StockAlert product={self.product_id} threshold={self.threshold}>"