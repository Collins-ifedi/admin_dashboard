...
async def approve_topup(
    ...
    """
    Approves a top-up: overwrites the amount with the actual received value,
    updates status, credits balance, notifies user, and logs action.
    """
    ...
    # OVERWRITE: Update TopUp record with the actual amount received
    # This ensures the database reflects reality, not the placeholder
    ...
    # Update Balance using the ACTUAL amount provided by admin
    ...
    notification = Notification(
        ...
        message=f"Your top-up has been approved. ${actual_amount:.2f} has been added to your balance.",
        is_read=False
    )
    db.add(notification)

    # Log Action with the confirmed amount
    log_msg = f"TopUp {topup_id} Approved (Amount: ${actual_amount:.2f})"
    await _log_admin_action(db, admin_id, "approve_topup", log_msg)
...
async def reject_topup(db: AsyncSession, admin_id: int, topup_id: int) -> TopUp:
    ...
    notification = Notification(
        type=NotificationType.TOPUP,
        user_id=topup.user_id,
        message=f"Your top-up request was rejected.", # Generic message since amount might be 0
        is_read=False
    )
    db.add(notification)
