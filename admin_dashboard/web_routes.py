...
async def login_for_access_token(
    ...
    """
    # This calls the logic in auth.py which checks ADMIN_PASSWORD (plain text)
    # and generates a secure token.
...
async def create_product(
    name: str = Form(...),
    price_usd: float = Form(...),  # UPDATED: Changed from 'price' to 'price_usd' to match Frontend
    ...
    - Accepts Form Data (multipart/form-data) to allow file uploads.
    - If 'file' is provided, parses .txt content and adds codes.
    - Uses 'services.add_product_with_codes' for upsert + deduplication logic.
    """
    raw_codes = ""
    if file:
        try:
    ...
    try:
        # Service handles checking for existing product, updating price, and adding codes
        result = await services.add_product_with_codes(
            db, admin_id, name, price_usd, raw_codes # UPDATED: Passing price_usd
        )
        return result
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
